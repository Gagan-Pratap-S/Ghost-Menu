...
function PrepBadge({ prep }: { prep: "fast" | "medium" | "slow" }) {
  const style = {
    fast:   { background: "rgba(34,197,94,0.12)", color: "#4ade80",  border: "1px solid rgba(34,197,94,0.2)" },
    medium: { background: "rgba(234,179,8,0.12)", color: "#fbbf24",  border: "1px solid rgba(234,179,8,0.2)" },
    slow:   { background: "rgba(239,68,68,0.12)", color: "#f87171",  border: "1px solid rgba(239,68,68,0.2)" },
  }[prep];
  return <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full" style={style}>{PREP_LABEL[prep]}</span>;
}
...
function HighDemandBadge() {
  return (
    <span className="absolute bottom-2 right-2 px-1.5 py-0.5 text-[9px] font-bold rounded-full"
...
function QtyControls({ qty, onInc, onDec }: { qty: number; onInc: (e: React.MouseEvent) => void; onDec: (e: React.MouseEvent) => void }) {
  return (
    <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
      <button onClick={onDec} className="w-6 h-6 flex items-center justify-center rounded-full font-bold text-xs active:scale-90 transition-transform duration-75 ease-out text-white"
      ...
      <button onClick={onInc} className="w-6 h-6 flex items-center justify-center rounded-full font-bold text-xs active:scale-90 transition-transform duration-75 ease-out text-white"
        style={{ background: "var(--brand,#f97316)" }}>+</button>
    </div>
  );
}
...
function AddButton({ onAdd }: { onAdd: (e: React.MouseEvent) => void }) {
  return (
    <button onClick={onAdd} className="w-6 h-6 flex items-center justify-center rounded-full text-white font-bold text-xs active:scale-90 transition-transform duration-75 ease-out"
      style={{ background: "var(--brand,#f97316)", boxShadow: "0 2px 8px rgba(249,115,22,0.35)" }}>+</button>
  );
}
...
export default function ItemCard({ item, onClick, variant = "list", priority = false, restaurantId, onView, recentOrderCounts }: Props) {
  ...
  // CUI-3: swipe-to-add (list variant only)
  ...
  if (variant === "quick") {
    return (
      <button ref={el => { cardRef.current = el; }} onClick={onClick} aria-label={item.name}
        className={`flex-shrink-0 w-36 snap-start group cursor-pointer focus:outline-none rounded-2xl ${isUnavail ? "item-unavailable" : ""}`}
      >
        <div className="rounded-2xl overflow-hidden transition-all duration-200 hover:-translate-y-0.5 active:scale-95" style={cardBase}>
          <div className="relative h-24" style={{ background: "rgba(30,41,59,0.8)" }}>
            ...
            {qty > 0 && <span className="absolute top-2 right-2 w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-bold text-white tabular-nums" style={{ background: "var(--brand,#f97316)" }}>{qty}</span>}
            {isUnavail && <div className="absolute inset-0 flex items-center justify-center" style={{ background: "rgba(2,6,23,0.6)" }}><span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Unavailable</span></div>}
          </div>
          <div className="p-2.5">
            <p className="font-display font-semibold text-white text-xs line-clamp-1 tracking-tight">{item.name}</p>
            <div className="flex items-center justify-between mt-1">
              <p className="text-orange-400 font-bold text-xs price tabular-nums">{formatPrice(item.price)}</p>
  ...
  if (variant === "grid") {
    return (
      <button ref={el => { cardRef.current = el; }} onClick={onClick} aria-label={item.name}
        className={`w-full group cursor-pointer focus:outline-none rounded-2xl ${isUnavail ? "item-unavailable" : ""}`}
      >
        <div className="rounded-2xl overflow-hidden transition-all duration-200 hover:-translate-y-0.5 active:scale-95" style={cardBase}>
          ...
          <div className="p-3">
            ...
            <div className="flex items-center justify-between mt-1.5">
              <div className="flex items-center gap-1.5">
                <p className="text-orange-400 font-bold text-sm price tabular-nums">{formatPrice(item.price)}</p>
  ...
  return (
    <button ref={el => { cardRef.current = el; }} onClick={onClick} aria-label={item.name}
      ...
      className={`w-full flex items-center gap-3 rounded-2xl p-3 text-left group cursor-pointer focus:outline-none transition-all active:scale-[0.98] ${isUnavail ? "item-unavailable" : ""}`}
      style={cardBase}
      onMouseEnter={e => { if (!isUnavail) (e.currentTarget as HTMLElement).style.border = "1px solid rgba(249,115,22,0.2)"; }}
    ...
    >
      <div className="relative w-[68px] h-[68px] flex-shrink-0 rounded-xl overflow-hidden" style={{ background: "rgba(30,41,59,0.8)" }}>
        ...
        {isUnavail && <div className="absolute inset-0 flex items-center justify-center" style={{ background: "rgba(2,6,23,0.7)" }}><span className="text-[8px] font-bold text-slate-400 uppercase tracking-wider leading-tight text-center">Un<br/>avail</span></div>}
        {qty > 0 && !isUnavail && <span className="absolute top-1 right-1 w-4 h-4 flex items-center justify-center rounded-full text-[9px] font-bold text-white tabular-nums" style={{ background: "var(--brand,#f97316)" }}>{qty}</span>}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mt-2">
          <div className="flex items-center gap-1.5">
            <p className="text-orange-400 font-bold text-sm price tabular-nums">{formatPrice(item.price)}</p>
            <PrepBadge prep={item.prep_time} />
            {isGoingFast && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{ background:"rgba(239,68,68,0.15)",color:"#f87171" }}>🔥</span>}
          </div>
          <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
            {item.tag && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full"
              style={item.tag.includes("Popular") ? { background:"rgba(249,115,22,0.12)", color:"#fb923c", border:"1px solid rgba(249,115,22,0.2)" } : { background:"rgba(251,191,36,0.1)", color:"#fbbf24", border:"1px solid rgba(251,191,36,0.2)" }}
            >{item.tag}</span>}
            {!isUnavail && (qty > 0 ? <QtyControls qty={qty} onInc={handleInc} onDec={handleDec} /> : <AddButton onAdd={handleAdd} />)}
          </div>
        </div>

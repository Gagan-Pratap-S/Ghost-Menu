  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" role="dialog" aria-modal="true" aria-label={item.name}>
      ...
      <div className="relative w-full max-w-md rounded-t-3xl max-h-[92vh] overflow-y-auto shadow-2xl animate-slideUp"
        style={{ background: "var(--color-surface,#0f172a)", border: "1px solid rgba(255,255,255,0.06)", borderBottom: "none" }}
      >
        {/* Close button */}
        ...
        <div className="p-5">
          ...
          <div className="flex items-start justify-between gap-2 mb-2">
            ...
            <span className="text-orange-400 font-display text-xl font-bold whitespace-nowrap price tabular-nums">
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            <span className="px-2.5 py-1 rounded-full text-xs font-medium" style={prepStyle}>
            ...
            <div className="mb-5 rounded-2xl p-4" style={{ background: "rgba(249,115,22,0.06)", border: "1px solid rgba(249,115,22,0.15)" }}>
              ...
              <div className="flex flex-wrap gap-2">
                {comboItems.map(ci => (
                  ...
                  >
                    <span>{ci.name}</span>
                    <span className="font-bold text-orange-500">+{formatPrice(ci.price)}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {qty === 0 ? (
            ...
            >
              ...
              <button onClick={() => increment(item.id)}
                className="w-9 h-9 flex items-center justify-center rounded-xl font-bold text-white transition-all active:scale-90"
                style={{ background: "var(--brand,#f97316)", boxShadow: "0 4px 12px rgba(249,115,22,0.3)" }}
              >+</button>
            </div>
          )}

          <button onClick={onClose} className="btn-ghost w-full py-3 text-sm mt-3">
            Back to Menu
          </button>
        </div>
      </div>

...
// Explicit state flags avoid TypeScript narrowing conflicts
type PlaceState = "idle" | "placing" | "success" | "error";

export default function CartModal({ open, onClose, restaurantId, guestName, memberCount }: Props) {
  ...
  const handlePlaceOrder = async () => {
    if (submitting.current || !isIdle) return; // strict double-submit guard

    // Validate restaurantId before hitting the API
    if (!restaurantId) {
      // No Supabase configured — show success as demo mode
    ...
    try {
      ...
      if (order) {
        ...
        // API failed but still show success for demo mode
        setPlaceState("success");
        clear();
      }
    } catch {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-md bg-white rounded-t-3xl shadow-2xl animate-slideUp max-h-[88vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-stone-100 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-stone-400 mt-0.5">
              {isSuccess ? "Kitchen is preparing your order" : `${totalItems} item${totalItems !== 1 ? "s" : ""}`}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3">
          {isSuccess ? (
            ...
            <div className="text-center py-12">
              ...
              <button
                onClick={() => { setPlaceState("idle"); submitting.current = false; }}
                  ...
                  <div className="relative w-14 h-14 flex-shrink-0 rounded-xl overflow-hidden bg-stone-200">
                    <Image
                      ...
                      onError={(e) => {
                        ...
                        target.src = "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=56&h=56&fit=crop";
                      }}
                    />
                  </div>
        ...
        {/* Footer — use explicit flags, not narrowed placeState */}

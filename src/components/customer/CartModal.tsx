...
export default function CartModal({ open, onClose, restaurantId, guestName, memberCount, tableNumber }: Props) {
  ...
  useEffect(() => {
    if (open) { setPlaceState("idle"); setOrderId(null); setOrderError(null); submitting.current = false; }
  }, [open]);
  ...
  const handlePlaceOrder = async () => {
    ...
    if (error) { setOrderError(error); setPlaceState("error"); submitting.current = false; return; }
    setOrderId(order?.id ?? null);
    setPlaceState("success");
    clear();
  };

  const sheetStyle = { background: "var(--color-surface)", border: "1px solid rgba(255,255,255,0.06)", borderBottom: "none" };
  const dividerStyle = { borderColor: "rgba(255,255,255,0.05)" };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-md rounded-t-3xl shadow-2xl animate-slideUp max-h-[88vh] flex flex-col" style={sheetStyle}>
        {/* Header */}
        <div className="flex items-center justify-between px-5 pt-5 pb-4 flex-shrink-0" style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          <div>
            <h2 className="font-display font-bold text-lg text-white tracking-tight">
              {isSuccess ? "Order Placed! 🎉" : "Your Cart"}
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">
              {isSuccess ? "Kitchen is preparing your order"
          ...
          <div className="flex items-center gap-2">
            {items.length > 0 && isIdle && (
              <button onClick={clear} className="text-xs text-red-400 hover:text-red-300 font-medium px-2 py-1 rounded-lg transition-colors"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3">
          {isSuccess ? (
            <div className="text-center py-12">
              ...
              {orderId && <p className="text-xs text-slate-600 mt-3 font-mono">#{orderId.slice(0,8).toUpperCase()}</p>}
              <button onClick={onClose} className="mt-6 w-full py-3.5 font-display font-bold rounded-2xl text-sm text-white transition-colors"
            ...
            <div className="text-center py-12">
              ...
              <button onClick={() => { setPlaceState("idle"); setOrderError(null); submitting.current = false; }}
                className="btn-primary mt-5 px-6 py-2.5 text-sm">Try Again</button>
            </div>
          ) : items.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              {items.map(item => (
                ...
                >
                  <div className="relative w-14 h-14 flex-shrink-0 rounded-xl overflow-hidden" style={{ background: "rgba(30,41,59,0.8)" }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-orange-400 font-bold text-sm price tabular-nums">₹{item.price * item.quantity}</p>
                  </div>
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => decrement(item.id)}
                      className="w-7 h-7 flex items-center justify-center rounded-lg font-bold text-sm text-white active:scale-90 transition-all"
                      style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)" }}
                    >−</button>
                    ...
                    >+</button>
                    <button onClick={() => remove(item.id)} aria-label={`Remove ${item.name}`}
                      className="w-7 h-7 flex items-center justify-center rounded-lg text-red-400 active:scale-90 transition-all ml-1"
                      style={{ background: "rgba(239,68,68,0.08)" }}
                    >
        ...
        {items.length > 0 && (isIdle || isPlacing) && (
          <div className="px-5 pb-6 pt-4 flex-shrink-0 space-y-3" style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
            <div className="flex items-center justify-between">
              <span className="font-display font-semibold text-slate-300">Total</span>
              <span className="font-display font-bold text-xl text-white price tabular-nums">₹{totalPrice}</span>
            </div>
            <button onClick={handlePlaceOrder} disabled={isPlacing} className="btn-primary w-full py-4 text-sm disabled:opacity-60 disabled:cursor-not-allowed disabled:transform-none">
              {isPlacing ? (

...
export default function CartModal({ open, onClose, restaurantId, guestName, memberCount }: Props) {
  ...
  // Shared scroll lock — ref-counted, safe alongside ItemModal
  useEffect(() => {
    if (open) { lockScroll(); return unlockScroll; }
  }, [open]);

  // Reset order state when modal re-opens
  useEffect(() => {
    if (open) { setPlaceState("idle"); setOrderId(null); }
  }, [open]);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (order) {
        ...
        // Supabase not configured — still show success for demo
        setPlaceState("success");
        clear();
      }
    } catch {
      ...
      <div className="relative w-full max-w-md bg-white rounded-t-3xl shadow-2xl animate-slideUp max-h-[88vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-stone-100 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-stone-400 mt-0.5">
              {placeState === "success" ? "Kitchen is preparing your order" : `${totalItems} item${totalItems !== 1 ? "s" : ""}`}
            </p>
          </div>
          <div className="flex items-center gap-2">
            {items.length > 0 && placeState === "idle" && (
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3">
          {placeState === "success" ? (
            <div className="text-center py-12">
              ...
              {orderId && (
                <p className="text-xs text-stone-400 mt-3 font-mono">Order #{orderId.slice(0, 8).toUpperCase()}</p>
              )}
            ...
            <div className="text-center py-12">
              ...
              <button onClick={() => setPlaceState("idle")}
        ...
        {items.length > 0 && placeState === "idle" && (
          <div className="px-5 pb-6 pt-4 border-t border-stone-100 flex-shrink-0 space-y-3">
            ...
            >
              {placeState === "placing" ? "Placing order..." : `Place Order · ₹${totalPrice}`}
            </button>

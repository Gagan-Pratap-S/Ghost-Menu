...
// Memoised — doesn't re-render when parent's search input changes
const MemoizedCategoryFilter = memo(CategoryFilter);

export default function MenuPage({ items, loading, kitchenStatus, guestName, memberCount, tableNumber, restaurantName, restaurantId, onItemClick, onViewItem }: Props) {
  ...
  const tableLabel = tableNumber && tableNumber !== "QR" ? ` · Table ${tableNumber}` : ` · ${memberCount} guest${memberCount !== 1 ? "s" : ""}`;
  ...
  return (
    <div className="min-h-screen pb-28 bg-stone-50">
            ...
            <section className="pt-5">
              ...
              <div className="flex gap-3 overflow-x-auto pb-1 px-4 snap-x scrollbar-none">
                {quickPicks.map((item, i) => (
                  <ItemCard key={item.id} item={item} onClick={() => handleItemClick(item)} variant="quick" priority={i === 0} restaurantId={restaurantId} onView={onViewItem} />
                ))}
              </div>
            </section>
          )}

          {/* 🔥 For You — renamed from Most Ordered, includes time context */}
          {topPicks.length > 0 && showSections && (
            <section className="pt-5 px-4">
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-display text-sm font-bold text-stone-900">🔥 For You</h2>
                <span className="text-xs bg-orange-50 text-orange-600 px-2 py-0.5 rounded-full font-medium">{timeContext}</span>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {topPicks.map((item, i) => (
                  <ItemCard key={item.id} item={item} onClick={() => handleItemClick(item)} variant="grid" priority={i < 2} restaurantId={restaurantId} onView={onViewItem} />
                ))}
              </div>
            </section>
          )}

          {/* Category filter — sticky, memoized to avoid re-renders on search input */}
            ...
            {fullMenu.length === 0 ? (
              ...
              <>
                ...
                <div className="space-y-2.5">
                  {fullMenu.map((item, i) => (
                    <ItemCard key={item.id} item={item} onClick={() => handleItemClick(item)} variant="list" priority={i < 3 && showSections} restaurantId={restaurantId} onView={onViewItem} />
                  ))}
                </div>
              </>
            )}
      ...
      {/* Sticky cart bar — replaces floating button */}
